...
class UnifiedLanguageBuilder implements Builder {
  ...
  String _generateAllCode(
    ...
    buffer.writeln(
      '// Found ${packages.length} slang packages with ${_getTotalUniqueLocales(packages)} unique locales',
    );
    buffer.writeln();
  ...
  int _getTotalUniqueLocales(List<SlangPackage> packages) {
    final allLocales = <String>{};
    for (final package in packages) {
      allLocales.addAll(package.availableLocales);
    }
    return allLocales.length;
  }
}
